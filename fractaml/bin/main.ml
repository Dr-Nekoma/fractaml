...
let color_generator(counter_value: float) : float =
  ...
  let adjust = List.nth color_adjusts (Int.rem (int_of_float magical_value) (List.length color_adjusts)) in
...
let draw (canvas: Html.canvasElement Js.t) =
   ...
   for row = 1 to int_of_float canvas_height do
     for col = 1 to int_of_float canvas_width do
       let real = ((float_of_int row) -. (canvas_width /. bounds)) *. (2.0 *. bounds /. canvas_width) *. !scale +. !centerX in
       let imaginary = ((float_of_int col) -. (canvas_height /. bounds)) *. (2.0 *. bounds /. canvas_height) *. !scale +. !centerY in 
